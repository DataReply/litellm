...
async def test_default_key_generate_params_object_permission_applied_when_absent(
    ...
    original_value = litellm.default_key_generate_params
    litellm.default_key_generate_params = {"object_permission": {"vector_stores": ["default-vs"]}}
...
async def test_update_key_non_budget_fields_allowed_for_internal_user(monkeypatch):
    ...
    mock_prisma_client.get_data = AsyncMock(return_value=mock_existing_key)
    mock_prisma_client.update_data = AsyncMock(return_value={"data": {"key_alias": "my-alias"}})
    mock_prisma_client.db.litellm_verificationtoken.find_unique = AsyncMock(return_value=mock_existing_key)
...
async def test_update_key_team_member_with_permission_can_update_non_budget(
    ...
    mock_prisma_client = AsyncMock()
    mock_prisma_client.get_data = AsyncMock(return_value=mock_existing_key)
    mock_prisma_client.update_data = AsyncMock(return_value={"data": {"key_alias": "renamed-by-member"}})
    mock_prisma_client.db.litellm_verificationtoken.find_unique = AsyncMock(return_value=mock_existing_key)
...
@pytest.mark.asyncio
async def test_enforce_email_prefix_on_key_alias_uses_general_settings_fallback(
    monkeypatch,
):
    monkeypatch.setattr(
        "litellm.proxy.management_endpoints.key_management_endpoints.get_ui_settings_cached",
...
async def test_prepare_key_update_data_applies_email_prefix_on_changed_user_id(
    ...
    mock_user_repository.find_by_id = AsyncMock(
        return_value=LiteLLM_UserTable(user_id="new-user", user_email="new.owner@example.com")
    )
    monkeypatch.setattr(
    ...
    result = await prepare_key_update_data(
        data=UpdateKeyRequest(key="hashed-token", user_id="new-user", key_alias="TestKey Name"),
        existing_key_row=existing_key,
        user_api_key_dict=UserAPIKeyAuth(user_id="caller", user_email="caller@example.com"),
...
async def test_prepare_key_update_data_falls_back_to_caller_email(monkeypatch):
    ...
    result = await prepare_key_update_data(
        ...
        existing_key_row=existing_key,
        user_api_key_dict=UserAPIKeyAuth(user_id="caller", user_email="caller.user@example.com"),
        prisma_client=None,
    )
...
async def test_update_key_non_budget_fields_reject_empty_normalized_alias(monkeypatch):
    ...
    mock_prisma_client = AsyncMock()
    test_hashed_token = "c1b2c3d4e5f6789012345678901234567890123456789012345678901234abcd"
